  ...
  _addItem: function (obj) {
    ...
    if (obj.overlay) {
      ...
      if (this.options.layerName) {
        elements.push(
          this._createNameElement("leaflet-control-layers-name", layerName)
        );
      }
      if (this.options.opacity) {
        elements.push(
          this._createRangeElement("leaflet-control-layers-range", opacity)
        );
      }
      if (this.options.color && !obj.uneditable) {
        elements.push(
          this._createColorElement("leaflet-control-layers-color", color)
        );
      }
      if (this.options.remove && !obj.uneditable) {
        elements.push(
          this._createRemoveElement("leaflet-control-layers-remove")
        );
      }
    } else {
      if (this.options.radioCheckbox) {
        elements.push(
          this._createRadioElement("leaflet-control-layers-selector", checked)
        );
      }
      if (this.options.layerName) {
        elements.push(
...
L.control.appearance = function (
  baseLayers,
  uneditableOverlays,
  overlays,
  options
) {
  return new L.Control.Appearance(
    baseLayers,
    uneditableOverlays,
    overlays,
    options
